...
@onready var attack_range:float = 10
...
@onready var los_horizontal_angle:float = 90
...
func tick(actor: Node, blackboard: Blackboard) -> int:
	check_player_raycast(blackboard, actor)
	return SUCCESS
...
func check_player_raycast(blackboard:Blackboard,actor: Node):
	var player_pos = blackboard.get_value("player_ref").get_global_position()
	var dist_to_player = (actor.get_global_position() - player_pos).length()
	...
	print(rad_to_deg(facing_dir_2d.angle_to(local_player_pos_2d)))
	# Check if:
	#	1. We see the player
	...
	#	3. We're inside our FOV
		...
		#horiz_angle > (los_horizontal_angle/2)):
			...
			#print("Out of FOV")
			#print("Horiz angle: ", horiz_angle)
		#return

	# 2. Check if we have LOS
	var dir_to_player = actor.get_global_position().direction_to(player_pos)
	var space_state = actor.get_world_3d().direct_space_state
	var origin = actor.get_global_position()
